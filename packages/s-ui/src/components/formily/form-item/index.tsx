...
} from '@formily/next/lib/__builtins__';
import { isVoidField } from '@formily/core';
import { connect, mapProps } from '@formily/react';
import { useFormLayout, FormLayoutShallowContext } from '@formily/next/lib/form-layout';

import { Balloon } from '@alicloud/console-components';

export type IFormItemProps = PropsWithChildren & {
  className?: string;
...
const useFormItemLayout = (props: IFormItemProps) => {
  ...
  return {
    ...
    tooltipIcon: props.tooltipIcon ?? layout.tooltipIcon ?? <QuestionCircleOutlinedIcon />,
  };
};
...
export const BaseItem: React.FC<IFormItemProps> = (props) => {
  ...
  const formatChildren =
    feedbackLayout === 'popover' ? (
      <Balloon needAdjust align="t" closable={false} trigger={children} visible={!!feedbackText}>
        <div
          className={cls({
      ...
      return (
        <span className={cls(`${prefixCls}-label-tooltip-icon`)}>
          <Balloon.Tooltip align="t" trigger={tooltipIcon}>
            <div dangerouslySetInnerHTML={{ __html: tooltip as string }} />
          </Balloon.Tooltip>
        </span>
      );
    }
...
export const FormItem: ComposeFormItem = connect(
  ...
  mapProps((props, field) => {
    ...
    return {
      ...
      extra: <div dangerouslySetInnerHTML={{ __html: props.extra || field.description }} />,
    };
  }),
);
